...
public class JdbcKnowledgeChunkRepository implements KnowledgeChunkRepository {
    ...
    private static final String DOCUMENT_VISIBLE_SQL = """
            SELECT COUNT(*) = 1
    ...
    @Override
    public boolean isDocumentVisible(Long documentId) {
        if (documentId == null) {
            return false;
        }
        Boolean visible = jdbcTemplate.queryForObject(DOCUMENT_VISIBLE_SQL, Boolean.class,
                documentId,
                dev.qcoding.businesscopilot.knowledgecopilot.retrieval.KnowledgeAccessContext.reviewerAllowed(),
                dev.qcoding.businesscopilot.knowledgecopilot.retrieval.KnowledgeAccessContext.adminAllowed());
