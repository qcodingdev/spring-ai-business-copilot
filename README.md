...
**V4 Report Copilot** now exposes a fictional evidence pack at `GET /api/report-copilot/sample-sources`, a client-evidence preparation API at `POST /api/report-copilot/source-previews`, and a structured generation endpoint at `POST /api/report-copilot/reports/generate`. Generation produces a `REVIEW_REQUIRED` candidate only: every factual section needs a valid source ID, metric values must exactly match a cited metric source, AI suggestions remain separate, and output is masked again. Confirmation and Markdown export follow in later V4 slices. It will not execute arbitrary SQL, modify tasks, or publish reports automatically.
