...
async function saveOnboardingChecklist(): Promise<void> {
  ...
  const items = lines(form.items).map((line) => {
    const [itemKey = '', title = '', guidance = '', ownerRole = '', requiredText = 'required', dueDaysText = '1'] = line.split('|').map((item) => item.trim())
    const dueInDays = Number.parseInt(dueDaysText, 10)
    return { itemKey, title, guidance, required: !['optional', '选办', 'false'].includes(requiredText.toLowerCase()), ownerRole: ownerRole || null, dueInDays: Number.isFinite(dueInDays) ? dueInDays : 0 }
  })
