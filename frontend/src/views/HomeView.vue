...
import MyTaskRunsPanel from '@/components/MyTaskRunsPanel.vue'
...
<template>
  ...
  <template v-else>
    <section class="workspace-hero workspace-hero--workbench">
      <div class="workspace-hero__copy">
        <p class="eyebrow">{{ t('common.workspaceEyebrow') }}</p>
        <h1>{{ t('common.welcome') }}，{{ session?.username }}</h1>
        <p>{{ t('common.welcomeDescription') }}</p>
        <div class="hero-assurance">
          <span :class="{ 'hero-assurance--warning': !session?.aiEnabled }"><i></i>{{ t(session?.aiEnabled ? 'common.aiConfigured' : 'common.aiNotConfigured') }}</span>
    ...
    <section class="home-section" aria-labelledby="module-heading">
      <div class="section-heading section-heading--home">
        <div>
          <h2 id="module-heading">{{ t('common.businessModules') }}</h2>
          <p class="field-hint">{{ t('common.businessModulesDescription') }}</p>
        </div>
