...
const pendingStatuses = new Set(['DRAFTED', 'NEEDS_HUMAN'])
const filteredQueue = computed(() => queue.value.filter((item) => {
  ...
  const matchesSearch = !query || `${item.externalReference} ${item.customerQuestion} ${item.category ?? ''}`.toLocaleLowerCase(locale.value).includes(query)
...
async function runAction(): Promise<void> {
  ...
  try {
    ...
    if (action === 'confirm' || action === 'cancel') {
      ...
      path = `/api/support-copilot/tickets/${encodeURIComponent(item.externalReference)}/record-manual-reply`
      init = { method: 'POST' }
    }
...
<template>
  <section class="support-review">
    ...
    <div class="queue-toolbar">
      ...
      <label>{{ t('support.queue.statusFilter') }}<select v-model="statusFilter"><option value="OPEN">{{ t('support.queue.openItems') }}</option><option value="">{{ t('common.all') }}</option><option value="DONE">{{ t('support.queue.completedItems') }}</option><option value="NEEDS_HUMAN">{{ statusLabel('NEEDS_HUMAN') }}</option><option value="DRAFTED">{{ statusLabel('DRAFTED') }}</option><option value="CONFIRMED">{{ statusLabel('CONFIRMED') }}</option><option value="CLOSED">{{ statusLabel('CLOSED') }}</option><option value="CANCELED">{{ statusLabel('CANCELED') }}</option></select></label>
    ...
    <div v-if="filteredQueue.length" class="review-queue-list">
      <article v-for="item in filteredQueue" :key="`${item.externalReference}-${item.draftId ?? 'manual'}`" class="review-queue-card">
        <div class="review-queue-card__heading"><div><small>{{ item.externalReference }}</small><h3>{{ item.customerQuestion }}</h3></div><StatusBadge :label="statusLabel(item.status)" :tone="statusTone(item)" /></div>
        ...
        <div class="button-row">
          <button v-if="item.draftId && ['DRAFTED','NEEDS_REVIEW'].includes(item.draftStatus ?? '')" class="button button--primary" type="button" :disabled="loading" @click="openReview(item)">{{ t('support.queue.reviewDraft') }}</button>
    ...
    <form v-if="selected && session" class="queue-review-editor" @submit.prevent="saveEdit">
      <div class="section-heading"><div><p class="panel-kicker">{{ selected.externalReference }}</p><h3>{{ t('support.queue.reviewTitle') }}</h3></div><StatusBadge :label="statusLabel(session.status)" tone="warning" /></div>
