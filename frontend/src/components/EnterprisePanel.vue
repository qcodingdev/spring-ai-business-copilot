...
const readEndpoint = computed(() => {
  const endpoints: Record<ModuleKey, Record<string, string>> = {
    ...
    hr: {
      ...
      authorization: '/api/resume-copilot/enterprise/question-bank',
...
function connectionRequest(): { path: string; body: Record<string, unknown> } {
  ...
  if (props.module === 'knowledge') {
    return {
      ...
      body: { ...common, rootReference: null, groupMapping: {}, defaultVisibility: 'INTERNAL' },
    }
  }
...
<template>
  <section class="panel">
    ...
    <form v-if="controlledAction" class="connection-form" @submit.prevent="actionConfirmOpen = true">
