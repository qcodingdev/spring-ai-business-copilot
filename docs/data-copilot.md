...
- 提供业务指标词典、字段中文解释和经审批的查询模板。
- 提供数据源健康检查、schema 变更快照与查询成本预检；计划无法解析时失败关闭。
- 查询执行具有稳定执行 ID、超时/行列/字节预算和显式取消能力。
- 已确认结果可受控导出 CSV/XLSX，或通过一次性交接引用进入 Report。
- 继续禁止写 SQL、任意数据库访问和商业 BI 看板。
