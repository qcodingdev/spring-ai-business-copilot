...
当前进度（2026-07-18）：用户决定不再按 v1.2～v1.6 分散发布，当前分支统一升级为 `2.0.0-SNAPSHOT`。可信确认、对象授权、五模块独立自动配置、审计、PostgreSQL/MySQL 外部只读查询底座、五模块纵向升级和中文工作台均已完成。影响真实操作的 Spring Security 7 CSRF 阻断已修复；Knowledge 已解决可变向量维度、混合维度检索、异步索引、服务端引用和多格式有界解析；Data 相对日期查询会转换为固定日期边界，不放宽普通函数安全策略。
...
### 8.5 Resume Copilot：简历评估助手

总目标：

帮助招聘团队基于已确认 JD 和脱敏简历生成逐条、可复核的证据化评估。
