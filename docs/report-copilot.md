...
> **V4 已实现，2.2 企业扩展已完成（2026-07）。** 本文档定义第四个业务模块的产品范围、业务闭环、安全边界和验收标准。
...
- 接入 Jira、会议纪要、Data 查询结果交接和 Support 质量统计。
- 支持周报、经营简报、项目状态、事故复盘和销售复盘模板。
- 定时任务只生成 `NEEDS_REVIEW`/草稿状态，不自动发布。
- 支持环比差异、来源异常以及 DOCX、PDF、PPTX 确定性导出和导出审计。
