...
> 状态：2.1 已形成质量、韧性和公网产品化基线；2.2 已完成正式版收口，发布记录以主干上的 `v2.2.0` 标签为准。
> 版本：`2.2.0`
> 最近审核：2026-07-29
...
上述能力改变了公开 API、数据库结构和企业使用闭环，不适合作为 2.1 修补项，因此以 `2.2.0-SNAPSHOT` 在 `feature/v2.2-enterprise-feedback` 开发；发布收口时去掉 `-SNAPSHOT`，正式版本为 `2.2.0`，标签为 `v2.2.0`。
