...
> **V5 已实现，2.2 企业扩展已完成（2026-07-28）。** 本文档记录第五个业务模块的产品范围、招聘合规边界、实现结构和验收标准。

## 业务价值

用户侧统一定位为 HR Copilot；内部继续复用 `resume-copilot` 模块、API 与数据库表。它不是简历打分器，而是帮助招聘人员生成岗位画像/JD、确认标准，并整理候选人材料证据、证据缺口和面试核验问题。
...
当前招聘流程为“岗位需求 → 岗位画像和 JD → 人工编辑 → 标准确认 → 单份简历证据分析 → 面试题 → 人工复核”。
...
- 候选人授权、撤回、有效期校验和按候选人绑定的 ATS 只读导入。
- 结构化面试题库、面试会话、证据化多人意见与待核实项汇总。
- JD/面试内容继续执行禁止评分、筛退和预测性招聘决定的确定性校验。
- 员工制度问答沿用 Knowledge 权限边界，并提供版本化入职清单和办事导航。
...
- 不做人脸、声音、情绪或性格推断。
- ATS 仅只读导入单个已授权候选人，不写回招聘网站、邮箱或员工系统。
- 不自动发送邀约、拒信或面试安排。
- 不持久化原始未脱敏简历。
- 不做背景调查、薪资预测或离职风险预测。
...
根据岗位需求生成可编辑的岗位画像和 JD 草稿，不自动确认标准。

### PUT /jobs/{jobId}/criteria

更新尚未确认的岗位标准；确认后不可原地修改，必须创建新版本。
